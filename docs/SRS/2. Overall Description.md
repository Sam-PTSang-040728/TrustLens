...
-\> \[Citation Parser + Normalizer\]  
-\> \[Metadata Lookup: Crossref/nguồn mở\]  
-\> \[NLP Similarity + Trust Score Engine\]  
...
| Metadata API      | Cung cấp thông tin học thuật phục vụ xác minh citation.                     | Không đăng nhập vào hệ thống; giao tiếp qua HTTP API.                              |
| Queue Worker      | Xử lý file và tính điểm bất đồng bộ.                                        | Truy cập job, file storage, database, metadata API.                                |
...
## 2.5. Constraints, Standards, Business Rules
