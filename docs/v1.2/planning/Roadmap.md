...
React/FastAPI, auth, role/permission, course/class/assignment, PDF/DOCX upload, real pipeline qua analyze, metadata verification, C1–C7 v1.2, report, PDF/DOCX/XLSX export, dashboard, admin user/provider/audit/AI diagnostics và một số unit test.

### Partial

Retry, process endpoint, scoring config management, report history, provider fallback, soft delete/retention, observability, frontend admin integration và test coverage.
...
- Xóa/ngừng dùng placeholder pipeline.
- Analyze/process/retry gọi cùng orchestration.
- Một state machine.
